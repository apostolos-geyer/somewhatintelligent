...
import type { MediaStorage, StorageResult } from "../src/lib/media-storage";
import {
  createRoadieMediaStorage,
  PUBLISHER_MEDIA_APPLICATION,
  type RoadieMediaClient,
} from "../src/lib/media-storage-roadie";
...
describe("Roadie MediaStorage adapter — put", () => {
  test("translates to the server-side roadie put and returns the minted referenceId as key", async () => {
    ...
    expect(calls[0]?.input).toEqual({
      ...
      size: 3,
      contentType: "image/png",
      application: { app: "publisher", resourceType: "media", resourceId: "media-row-id" },
      body,
    });
  });
  ...
  test("honors the caller's application tuple", async () => {
    ...
    const storage = createRoadieMediaStorage(client, {
      application: { app: "publisher", resourceType: "text-hero" },
    });
    await storage.put({
      key: "m1",
      body: bodyStream(),
    ...
    expect(calls[0]?.input).toMatchObject({
      application: { app: "publisher", resourceType: "text-hero", resourceId: "m1" },
    });
  });
...
describe("port surface — no Roadie vocabulary crosses it (INV-MEDIA-1 / D10)", () => {
  ...
  // Type-level assertion. Publisher's tsconfig excludes __tests__ from tsgo;
  // the enforced static guard lives in src/lib/media-storage-roadie.ts.
  type Equal<A, B> =
    (<T>() => T extends A ? 1 : 2) extends <T>() => T extends B ? 1 : 2 ? true : false;
  type Assert<T extends true> = T;
  type _MethodsAreExactlyThePort = Assert<Equal<keyof MediaStorage, "put" | "read" | "delete">>;
  type _ErrorIsThePortUnion = Assert<
