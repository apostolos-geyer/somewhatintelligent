/**
 * Roadie adapter for the store's private MediaStorage port (RFC-0001 D10).
 * ALL Roadie translation for the store is confined to this module; swapping
 * in a direct-R2 implementation changes no caller. Wire it over the existing
 * client factory in `@/lib/roadie`:
 *
 *   const media = createRoadieMediaStorage(getRoadie(), {
 ...
 */
import type { Result } from "@si/roadie-service/client";

import type { MediaStorage, StorageResult } from "./media-storage";

/**
 * The slice of Roadie's consumer SDK the adapter uses. Hand-mirrored from
 * Roadie's wire contract (workers/roadie.md §API Contract) rather than
 * derived from the RPC client type: Workers RPC intersects every return
 * value with `Disposable` and carries a broader per-method error union than
 * this adapter needs, so a plain structural type keeps tests stubbable with
 * ordinary objects instead of fighting the RPC surface.
 */
export interface RoadieMediaClient {
  put(input: {
    hash: string;
    size: number;
    contentType: string;
  ...
  getReadUrl(input: {
    ...
    Result<
      { url: string; expiresAt: number; cached: boolean },
      "reference_not_found" | "not_ready" | "deleted" | "invalid_lifetime"
    >
  >;
  removeReference(input: { referenceId: string }): Promise<Result<null>>;
}
...
/** Canonical Roadie application tuple for store media (matches upload.functions.ts). */
export const STORE_MEDIA_APPLICATION = {
  app: "storefront",
...
export type PortHasNoRoadieVocabulary = AssertNever<Extract<PortSurface, RoadieVocabulary>>;
