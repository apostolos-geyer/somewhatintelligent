...
//
// We pass identity's own origin so every request stays same-origin from the
// browser's perspective; `workers/identity/src/routes/api/$.ts` then proxies
...
// In SSR (`window` undefined), fall back to the wrangler-baked IDENTITY_URL.
const baseURL =
  typeof window !== "undefined" ? window.location.origin : import.meta.env.IDENTITY_URL;
