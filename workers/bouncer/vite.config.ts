...
// si-guestlist-staging), so the stub workers below must carry the same
  ...
  plugins: [
    cloudflareTest(() => ({
      ...
      miniflare: {
        ...
        // specs assert against); identity shares a generic stub — bouncer's
        // dispatch/proxy logic doesn't care which app responds. Names carry
        // the worker prefix + `-staging` suffix so they match the
        // `<prefix>-<service>-staging` service-binding targets in wrangler.jsonc.
        workers: [
