...
beforeEach(() => {
  // Same routes as microfrontend-template/wrangler.jsonc, opted into `vmf`
  // mode. The CF template's transformation pipeline is what these parity
  // tests verify; under our bouncer it lives behind the `vmf` mode gate
  // (default mode is `passthrough`, which would skip the transformations).
  env.ROUTES = {
    smoothTransitions: true,
    routes: [
      { binding: "APP1", path: "/app1", preload: true, mode: "vmf" },
      { binding: "APP2", path: "/app2", mode: "vmf" },
    ],
...
describe("Microfrontend Router", () => {
  ...
  describe("redirect handling", () => {
    it("rewrites redirect Location headers", async () => {
      const response = await SELF.fetch("https://example.com/app2/redirect-test", {
        redirect: "manual",
      });
  ...
  describe("preload script", () => {
    it("serves preload script at /__mf-preload.js when other routes have preload enabled", async () => {
      ...
      const response = await SELF.fetch("https://example.com/app2/__mf-preload.js");
