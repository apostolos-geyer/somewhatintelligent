    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          ...
          {/* Pad the inner flex container by the OS safe-area insets so the
              sidebar header (logo / app switcher) and footer (account /
              theme) don't draw under the iOS dynamic island, notch, or home
              indicator when launched as an installed standalone PWA. The
              parent SheetContent keeps `bg-sidebar` so the surface still
              extends edge-to-edge — only content respects the inset. */}
          <div
  ...
  return (
    ...
    >
      ...
      <div
        ...
        {...props}
      >
        {/* Same safe-area treatment as the mobile sheet path — relevant on
            iPad / Mac standalone PWAs where the top status bar overlaps the
            fixed sidebar at `inset-y-0`. The bg keeps filling the full
            container; only children respect the inset. */}
        <div
          data-sidebar="sidebar"
