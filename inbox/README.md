...
This is the `somewhatintelligent.ca` instance, vendored as a standalone,
separately-deployed project — Worker `agentic-inbox-si`, domain
`mail.somewhatintelligent.ca`, R2 bucket `agentic-inbox-si`. `wrangler.jsonc`
is a single-instance config (no named `env.*` blocks); `wrangler deploy` (no
`--env`) always ships this Worker.

### Configuration

1. Domain is set in `wrangler.jsonc` (`DOMAINS`) — already `mail.somewhatintelligent.ca`
2. Create the R2 bucket: `wrangler r2 bucket create agentic-inbox-si`
...
See [`scripts/README.md`](./scripts/README.md) for the one-time Access +
Email Routing setup commands (`setup:access`, `setup:email`).
