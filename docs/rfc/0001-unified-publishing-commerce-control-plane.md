...
Resolutions made while building exec-plan 0004 that this RFC left open or under-specified. Each is a design commitment, not a preference — recorded here so the topology stays legible.

| Date       | Resolution                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-07-19 | **Media-route disambiguation is by route prefix, not id-namespacing (D10).** Publisher owns the bare `/media/:id` prefix — every `PublicMediaRef.href` from `PublisherPublic` emits `/media/:id`, and Site's `GET /media/:mediaId` forwards straight to `PublisherPublic.openPublishedMedia`. Store owns `/api/store/media/:id`, served by Store through bouncer's `/api/store` mount. The owner is decided by the path; no shared id namespace exists.                                                                                                                                                                                               |
| 2026-07-19 | **Preview is an HMAC-signed draft POST, with no draft bindings anywhere (D14).** Operator POSTs the rendered draft document to a POST-only Site `/__preview` route signed with a shared `PREVIEW_SIGNING_SECRET` (HMAC-SHA256, ~120s TTL). Site holds no draft binding (INV-SITE-1) and Operator holds no Site binding (INV-OP-2); the draft travels in the request body. Site re-validates the document, renders it through the public view pipeline, and returns `noindex` + `no-store`.                                                                                                                                                            |
| 2026-07-19 | **Deletion tokens are hashed intents with impact-hash drift rejection, implemented per worker (D8).** `@si/contracts` exports only the deletion DTOs (`DeletionImpact`/`DeletionPlan`/`ConfirmDeletionInput`/`DeletionError`); Store (`workers/store/src/lib/operator.ts`) and Publisher (`workers/publisher/src/operator/writes.ts`) each own their token machinery — plan persists only the token's SHA-256 hash + a canonical impact hash bound to operator/action/target, and confirm re-derives impact from current data and rejects on hash drift. Per-worker copies were chosen over a shared execution helper because the two schemas differ. |
| 2026-07-19 | **RPC-tunneled upload streams are re-length-fixed with `FixedLengthStream` (D10).** Server-side `roadie.put` (the `MediaStorage` write path) receives a `ReadableStream` over RPC without length metadata, which R2 `put` rejects. Roadie pipes it through `new FixedLengthStream(input.size)` using the already-validated declared size (`workers/roadie/src/methods/upload.ts`), so no buffering is added and R2's SHA-256 check still guards the bytes.                                                                                                                                                                                            |
| 2026-07-19 | **Local dev serves media bytes from a dev-only Roadie blob route.** In `ENVIRONMENT === "development"`, `MediaStorage` read URLs point browsers at Roadie's `/__dev/blob/:id` route (`workers/roadie/src/index.ts`) instead of a presigned S3 URL, so bytes in the miniflare R2 sim are readable back over HTTP and seeds can push bytes in over PUT without a service binding. The route is never mounted outside development and rejects ids containing path separators.                                                                                                                                                                            |
