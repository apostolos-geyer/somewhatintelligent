...
| `STRIPE_SECRET_KEY`                               | `src/auth-config.ts` (`buildStripeOptions`) → `@somewhatintelligent/auth`'s `stripe` plugin gate | `env-init` (blank placeholder)                       | secret (packages/secrets), `provided`/optional — **unset until Stripe onboarding** (`workers/guestlist/scripts/billing.ts`); the better-auth `stripe` plugin is omitted from the plugins array entirely unless this AND `STRIPE_WEBHOOK_SIGNING_SECRET` are both set |
...
runtime: `workers/guestlist/scripts/billing.ts` (`bun run billing fetch` /
`sync` / `validate`, wrapping `runBillingCli` from
`@somewhatintelligent/stripe/cli`) reads it straight from `process.env` — not
from `.dev.vars` — to talk to a real Stripe test/live account when onboarding.
Without it, `fetch` takes the offline stub path and never imports the Stripe
SDK; the generated `src/billing.gen.ts` (product/price id maps) is
consumer-owned and gitignored, rewritten on every successful non-dry-run
`sync`/`validate`.
...
| `STRIPE_PUBLISHABLE_KEY`        | `src/lib/checkout.functions.ts` (`getCheckoutConfig` gate), `src/routes/_app/checkout.index.tsx` (`loadStripe`, via vite `CLIENT_VARS`) | `env-init` (blank placeholder; set a `pk_test_…` to enable the Payment Element locally)   | wrangler var (blank until Stripe onboarding) — client-safe `pk_…`; `getCheckoutConfig` requires it alongside the secrets before the card form renders                                                              |
| name                            | consumed by                                                                                                                             | dev source                                                                                | staging + production source                                                                                                                                                                                        |
| ------------------------------- | -------------------------------------------------------------------------------------------------                                       | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `ENVIRONMENT`                   | `src/lib/platform.ts`                                                                                                                   | `env-init` via `PLATFORM_DEV_VARS` (`development`)                                        | wrangler var                                                                                                                                                                                                       |
| `PUBLIC_BASE`                   | `src/lib/basepath.ts` → `src/router.tsx` (client mount)                                                                                 | `env-init` (`/` — dev-direct has no mount)                                                | wrangler var (`/shop`) — build-time FALLBACK for the client mount; bouncer's runtime `si-mount` meta wins, and the mount rides the router `rewrite` option (`mountRewrite`), not `basepath`                        |
| `STORE_URL`                     | `src/lib/platform.ts` (`expectedHost`), header sign-in link                                                                             | `env-init` (`https://store.somewhatintelligent.localhost`); client prefers `PORTLESS_URL` | wrangler var — the store's own PUBLIC address (`.../shop`)                                                                                                                                                         |
| `IDENTITY_URL`                  | `src/lib/auth-client.ts` (SSR `/api` base), sign-in link                                                                                | `env-init` via `PLATFORM_DEV_VARS`                                                        | wrangler var — identity's PUBLIC address; SSR derives the bare same-host origin via `new URL(...).origin` for `/api`                                                                                               |
| `STORE_LIVE`                    | `src/lib/config.ts` → `src/lib/store-gate.ts` (launch gate: routes, header, public product reads)                                       | `env-init` (`false` — landing shows locally; admins bypass)                               | wrangler var (`"true"`/`"false"`, per env) — while `"false"` the storefront is admin-only and non-admins get the pre-launch landing/`/welcome`; unset parses to closed                                             |
| `AUTH_DOMAIN`                   | (platform helpers)                                                                                                                      | `env-init` via `PLATFORM_DEV_VARS` (`LOCAL_AUTH_DOMAIN`)                                  | wrangler var                                                                                                                                                                                                       |
| `BNC_ATT_KID`                   | kit dev-envelope stamper (`src/lib/platform.ts`)                                                                                        | `env-init` (`LOCAL_BNC_ATT_KID` = `dev`)                                                  | absent (bouncer mints; app verifies with committed public keys)                                                                                                                                                    |
| `BNC_ATT_PRIV`                  | kit dev-envelope stamper                                                                                                                | `env-init` (`LOCAL_BNC_ATT_PRIV`)                                                         | absent (bouncer holds it)                                                                                                                                                                                          |
| `STRIPE_SECRET_KEY`             | `src/lib/stripe-webhook.ts` (`stripeConfigured`, `handleStoreStripeWebhook`)                                                            | **absent — not seeded by env-init** (see Known inconsistencies)                           | secret (packages/secrets), `provided`/optional — gates webhook signature verification together with `STRIPE_WEBHOOK_SIGNING_SECRET`; `stripeConfigured()` requires both to be set or the webhook route returns 503 |
...
`import.meta.env.{STORE_URL,IDENTITY_URL,AUTH_DOMAIN,ENVIRONMENT,PUBLIC_BASE,STORE_LIVE}`
...
| `WORKER_VERSION` / `WORKER_COMMIT`                     | `/__version` on bouncer/guestlist/roadie/promoter/identity — `@somewhatintelligent/kit/version`'s `versionInfo()` no longer reads these off `env` itself; each worker reads them off its own env and passes them through `handleVersionRequest(...)`'s `overrides: { version, commit }` (precedence `overrides > pkg > '0.0.0-dev'/'unknown'`) | absent — endpoint falls back to `0.0.0-dev` / `unknown`                                                               | **CD:** injected as `--var` at ship time by `scripts/deploy-worker.sh` (deploys) and `scripts/generate-preview-tasks.sh` (preview version uploads, so promoted versions carry them). NOT `wrangler.jsonc` vars — never in Env types.                                      |
