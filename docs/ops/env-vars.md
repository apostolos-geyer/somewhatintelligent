...
seeder only writes the keys that must *differ* from staging for local
correctness — everything else silently takes its staging value in local dev.
That is why a "dev source" cell can read *wrangler top-level (staging), not
overridden*: the key is real in dev, it just isn't in `.dev.vars`.
...
| `dev-config` | A `LOCAL_*` constant or `PLATFORM_DEV_VARS` in `scripts/dev-config.ts` — the *value* a seeder writes. |
...
| `IDENTITY_URL` | `src/index.ts`, `src/session.ts` | `env-init` (`dev-config` `LOCAL_IDENTITY_URL`) | wrangler var |
| `SPROUT_URL` | routing / redirects | absent (bouncer not in dev path) | wrangler var |
...
| `ROUTES` | `src/routes.ts` (`matchRoute`) | absent (portless routes dev) | wrangler var (JSON route table; production adds marketing apex/www + `/hub`) |
...
| `BETTER_AUTH_URL` | `src/auth-config.ts`, `src/index.ts` | `env-init` (`LOCAL_IDENTITY_URL`) | wrangler var |
| `IDENTITY_URL` | `src/auth-config.ts` | `env-init` (`LOCAL_IDENTITY_URL`) | wrangler var |
| `AUTH_DOMAIN` | `src/auth-config.ts` | `env-init` (`LOCAL_AUTH_DOMAIN`) | wrangler var (`.sproutportal.ca`) |
| `EMAIL_FROM` | `src/auth-config.ts` | `env-init` (`identity@resend.dev`) | wrangler var — staging `identity@resend.dev`, production `Sprout <hello@sproutportal.ca>` |
...
isn't in the path on `*.sproutportal.localhost`), so it carries the dev signing
...
| `IDENTITY_URL` | `src/lib/platform.ts`, `src/lib/return-to.ts`, sidebar/modals | `env-init` via `PLATFORM_DEV_VARS`; client bundle prefers `PORTLESS_URL` | wrangler var |
| `AUTH_DOMAIN` | `src/lib/return-to.ts` (redirect guard) | `env-init` via `PLATFORM_DEV_VARS` (`LOCAL_AUTH_DOMAIN`) | wrangler var |
| `MARKETING_URL` | `src/components/dashboard/app-sidebar.tsx` | wrangler top-level (staging), not overridden — reaches the client bundle via the vite vars overlay | wrangler var — staging `…workers.dev`, production `https://sproutportal.ca` |
| `SPROUT_URL` | portal links | wrangler top-level (staging), not overridden | wrangler var |
...
`import.meta.env.{IDENTITY_URL,MARKETING_URL,SPROUT_URL,AUTH_DOMAIN,ENVIRONMENT}`
are baked into the client bundle at build time by `vite.config.ts` (allowlisted
in `CLIENT_VARS`). The overlay logic there is gated by `GREENROOM_BUILD` /
`CLOUDFLARE_ENV` (bottom table) so a dev `.dev.vars` never leaks into a shipped
bundle.

## marketing (`workers/marketing`)

Astro (Cloudflare adapter). **No `env-init` seeder** — ships no `.dev.vars`.

| name | consumed by | dev source | staging + production source |
| --- | --- | --- | --- |
| `MARKETING_URL` | Astro islands via `import.meta.env` (`astro.config.mjs` `CLIENT_VARS`) | wrangler top-level (staging), not overridden | wrangler var — staging `…workers.dev`, production `https://www.sproutportal.ca` |
| `IDENTITY_URL` | Astro islands via `import.meta.env` | wrangler top-level (staging), not overridden | wrangler var |
...
The branded portal app. Dev-direct (self-mints its envelope). Has local D1,
a Durable Object, a queue, Analytics Engine, and the remote-only AI / Vectorize
/ Browser bindings. Bindings (not env vars) are inventoried in
`docs/ops/02a-wrangler-binding-inventory.md`.

| name | consumed by | dev source | staging + production source |
| --- | --- | --- | --- |
| `ENVIRONMENT` | `src/lib/platform.ts` | `env-init` (`development`) | wrangler var |
| `SPROUT_URL` | `src/room-server.ts`, `src/lib/platform.ts`, `src/lib/hub.functions.ts`, … | `env-init` (`https://sprout.sproutportal.localhost`); client bundle prefers `PORTLESS_URL` | wrangler var — staging `sprout-staging.sproutportal.ca`, production `https://sproutportal.ca` (apex) |
| `IDENTITY_URL` | `src/lib/platform.ts` | `env-init` (`LOCAL_IDENTITY_URL`) | wrangler var |
| `AUTH_DOMAIN` | auth/session | `env-init` (`LOCAL_AUTH_DOMAIN`) | wrangler var |
| `BRAND_RESOLUTION` | `src/lib/brand-resolution.ts` (build-time client define) | `env-init` (`subdomain`) | wrangler var — **staging `path`, production `subdomain`** |
| `BNC_ATT_KID` | kit dev-envelope stamper | `env-init` (`LOCAL_BNC_ATT_KID` = `dev`) | absent (bouncer mints) |
| `BNC_ATT_PRIV` | kit dev-envelope stamper | `env-init` (`LOCAL_BNC_ATT_PRIV`) | absent (bouncer holds it) |
| `CF_ACCOUNT_ID` | `src/lib/realtime.ts` | absent (realtime seam degrades to no-op) | wrangler var (CF account id) |
| `RTK_APP_ID` | `src/lib/realtime.ts` | absent (realtime degrades) | wrangler var — staging/production RealtimeKit App id (non-secret) |
| `RTK_API_TOKEN` | `src/lib/realtime.ts` | absent (realtime degrades) | **secret (packages/secrets)** — `provided`/optional, Realtime-scoped CF API token. Provision the App id/token with `scripts/provision-realtimekit.ts`. |
...
| --- | --- | --- | --- |
| `CLOUDFLARE_API_TOKEN` | `wrangler` (deploy, `secret put`), `scripts/provision-realtimekit.ts`, `packages/secrets/src/run.ts` | absent — local wrangler uses miniflare; remote bindings / solo-mode / `bun run secrets` need this or `wrangler login` | **CD:** RWX vault `greenroom_deploy.secrets.CLOUDFLARE_API_TOKEN` (`.rwx/deploy.yml` + `.rwx/release-please.yml` `deploy-env` alias). Must be minted **from the Sprout CF account** and cover both envs (Workers Scripts Edit + D1 Edit). |
| `CLOUDFLARE_ACCOUNT_ID` | `wrangler`, `packages/secrets`, `provision-realtimekit.ts` (falls back to `platformDeployConfig.cloudflareAccountId`) | optional | **CD:** RWX vault `greenroom_deploy.secrets.CLOUDFLARE_ACCOUNT_ID`. Distinct from the `CF_ACCOUNT_ID` **worker var** (same value, different surface: this one is a deploy/CLI env, `CF_ACCOUNT_ID` is read at runtime by sprout). |
...
| `GREENROOM_BUILD` | `workers/{identity,sprout}/vite.config.ts`, `workers/marketing/astro.config.mjs` | unset (dev overlays `.dev.vars` into the client bundle) | Set to `1` for any shipped build (`deploy:staging` scripts) so seeded `.dev.vars` can never leak into a shipped bundle |
| `CLOUDFLARE_ENV` | same vite/astro configs (selects the `env.<name>.vars` block) + `wrangler deploy --env` | unset | Set to `production` for prod builds (`deploy:production` scripts); staging leaves it unset and relies on `GREENROOM_BUILD=1` |
| `NODE_ENV` / `VITE_*` | vite/astro `build` task `env` allowlist | per toolchain | passthrough (build inputs) |
| `PORTLESS_URL` | `workers/{identity,sprout}/vite.config.ts` (overrides `IDENTITY_URL`/`SPROUT_URL` in the client bundle to the branch-prefixed portless origin) | set by `portless` (`bun run dev`) | n/a |
...
- **`docs/secrets.md` still says `.platform.localhost`.** The dev cookie/apex
  domain is `.sproutportal.localhost` (`scripts/dev-config.ts`
