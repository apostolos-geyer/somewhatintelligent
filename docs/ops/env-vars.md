...
from the locked `si_deploy` RWX vault — `CLOUDFLARE_API_TOKEN`,
...
| `GH_TOKEN`                                             | `scripts/rwx-github-deployment.sh` (fleet + per-worker GitHub Deployment records, deploy summary comment)                | n/a                                                                                                                   | **CD:** RWX vault `si_deploy.github-apps.rwx-automation-somewhatintelligent.token` (`deployments: write`; `pull-requests: write` for the summary comment — degrades to log lines without it)                                         |
| `GITHUB_TOKEN`                                         | `.rwx/release-please.yml` (`rp-env` alias — release-please CLI + tag API)                                                | n/a                                                                                                                   | **CI:** same `rwx-automation-somewhatintelligent` App token from the vault                                                                                                                                                           |
