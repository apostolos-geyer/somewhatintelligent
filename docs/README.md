# Sprout platform docs

Documentation for the Sprout fork of the platform monorepo (bouncer, guestlist,
roadie, promoter, identity, sprout, and supporting packages).
...
| [`cloudflare-realtimekit.md`](cloudflare-realtimekit.md)            | RealtimeKit research synthesis + implementation plan for real-time features in this repo.                                                                                                                                       |
...
| [`runbooks/SECRETS.md`](runbooks/SECRETS.md)                        | Codified secret provisioning via `@greenroom/secrets` (`bun run secrets <env>`) — the manifest, sources, and the attestation keypair flow.                                                                                      |
...
The [`ops/`](ops) and [`sprout/`](sprout) directories hold the CI/CD maintenance
specs and the Sprout product build docs respectively.
