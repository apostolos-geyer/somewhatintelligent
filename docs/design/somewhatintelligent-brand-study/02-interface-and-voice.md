...
- `Wearable field notes.` — tasteful but bloodless.
- `Printed in small runs.` — keep as factual fulfillment metadata, not the thesis.
- `Nothing live yet — check back soon.` — tells the visitor to leave.
...
When the direction is approved, implement in this order:

1. homepage shell and global navigation;
2. FRIEND-001 product story and purchase page;
3. writing index + article template;
4. software registry + subscription entry points;
5. about/person record;
6. account and admin visual integration.

Do not begin by recoloring shared components. Establish the homepage thesis, imagery, type hierarchy, and product page first; those surfaces will reveal which design tokens and primitives actually need to change.
