...
| G3  | **At most one open checkout session per user, via supersede-on-create.** `createCheckoutSessionCore` — after pricing/customer validation, before reserving new stock — selects the caller's supersedable prior attempts (`status='pending' AND paymentStatus IN ('unpaid','processing') AND stripeCheckoutSessionId IS NOT NULL`) and, for each, calls `stripe.checkout.sessions.expire` (injected `expireSession` dep, same pinned-`apiVersion` client). **Expire-at-Stripe is the release authority**: Stripe only expires an OPEN session, so a successful expire _proves_ the session can never be paid → release+cancel that order synchronously in one gated `db.batch` (reusing `reconcile.ts`'s `releaseAndCancel` idiom exactly — stock re-increment gated on the order still unpaid/processing, then `paymentStatus→'expired'`/`status→'cancelled'` CASE-gated, no ledger row; the later `checkout.session.expired` webhook no-ops through the same gates). A **throw** means the session is not open (complete/already-expired — a paid webhook may be in flight): the order is left completely untouched (`store.stripe_checkout.supersede_skipped`), never released on D1 state alone — the webhook/cron settle it. A successful supersede logs `store.stripe_checkout.superseded`. Orphans (`stripeCheckoutSessionId IS NULL`) belong to the cron (Track D5/D6) and are excluded. **A supersede failure never fails the new checkout** — control falls through to the new reservation regardless. | Without this, a buyer who initiates → backs out → re-initiates leaves N live sessions + N pending orders, each holding reserved stock for up to the full 30-min TTL: false out-of-stock for others, self-block on the last unit their own abandoned reservation holds, and — with redirect methods (Klarna) — an abandoned session that **remains payable at Stripe**, completable even after the buyer paid a second session (a visible double charge). Supersede, never block: the release authority is Stripe's own OPEN-only `expire`, so a released order can never be one the buyer later pays; failures defer to the webhook/cron rather than releasing blind. Reuses the new-order-per-attempt model (G2) unchanged — supersede cancels the _prior_ attempt, the new attempt is still a fresh order. |
...
- 2026-07-13 — At-most-one-open-checkout-session-per-user (new Track G3):
  supersede-on-create in `createCheckoutSessionCore` — a new `expireSession`
  dep expires the caller's prior open sessions before reserving new stock, and
  a successful expire (Stripe's OPEN-only authority) releases+cancels that
  prior order via `reconcile.ts`'s `releaseAndCancel` (now exported); a throw
