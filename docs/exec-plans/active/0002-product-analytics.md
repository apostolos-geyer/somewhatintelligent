...
| 12  | LOW      | `reset()` only on the sign-out and delete-account buttons → misses session expiry, revocation, and store logout (shared-computer leak).                                                                                                                                   | `_dashboard.tsx` (`handleSignOut`), `delete-account-dialog.tsx:34`                | Transition-guarded `reset()` inside the shared bridge.                                                    |
