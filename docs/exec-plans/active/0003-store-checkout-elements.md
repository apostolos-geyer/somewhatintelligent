...
sequenceDiagram
    ...
    alt any line's stock guard failed (changes=0)
        ...
        St->>Api: checkout.sessions.create(ui_mode:"elements", mode:"payment",<br/>customer, price_data×N, metadata:{orderId}, expires_at:+30min, return_url)
...
| B3  | Shipping is folded into the Session as a second `price_data` line item (`title: "Shipping"`, `unit_amount: shippingCents`), not Stripe's `shipping_options` | Keeps `calculateShipping`'s flat-rate/free-over-threshold logic (`lib/config.ts`) as the single shipping-cost authority, unchanged from `placeOrder`'s totals.                               |
